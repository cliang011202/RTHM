...
%% --- 自检判据（用 TowerTopLoad 而不是 sum-F_aero）---
mask = t > simu.rampTime + 5;          % 跳过 ramp + 5 s 沉降

% TowerTopLoad 的 6 列：[Fx Fy Fz Mx My Mz] @ tower top
fprintf('\n========== TowerTopLoad 自检 (mean over [ramp+5, end]) ==========\n');
ttMean = mean(ttLoad(mask,:), 1);
ttStd  = std(ttLoad(mask,:),  0, 1);
ttLbl  = {'Fx','Fy','Fz','Mx','My','Mz'};
for k = 1:6
  fprintf('  ttLoad %2s : mean = %+.3e   std = %.2e\n', ttLbl{k}, ttMean(k), ttStd(k));
end

%% Case A1/A2 相位判据
% 重构假信号（用与 Sine Wave block 完全相同的参数）
% A1: px = 5 * sin(2π*0.08 t),    A2: qy = deg2rad(3) * sin(2π*0.05 t)
isCase = 'A0';   % 'A0' / 'A1' / 'A2'   <-- 跑前手改

switch isCase
  case 'A1'
      px = 5 * sin(2*pi*0.08 * t);
      vx = gradient(px, t);                                    % surge 速度
      cc = corrcoef(vx(mask), ttLoad(mask,1));
      fprintf('\n[A1] corr(surge_vel, ttFx) = %+.3f  (期望 < 0：顺风走 -> 推力减小)\n', cc(1,2));

  case 'A2'
      qy = deg2rad(3) * sin(2*pi*0.05 * t);
      cc = corrcoef(qy(mask), ttLoad(mask,5));
      fprintf('\n[A2] corr(qy, ttMy) = %+.3f  (期望 < 0：俯仰角与气动俯仰力矩反相)\n', cc(1,2));
end
%% --- 7. 画图 ---
...
%% --- 8. 保存为 level1_<case>.mat（供 analyzeLevel1.m 使用）---
% 跑前确认 isCase 是 'A0' / 'A1' / 'A2'，且 Simulink 里 Sine Wave 配置匹配
saveFile = sprintf('level1_%s.mat', isCase);
save(saveFile, 't', 'ttLoad', 'F_aero', 'data', 'c', ...
               'windV', 'rotSpd');
% simu 是 handle 类，存进去会带一堆引用，单独存它的几个标量字段
simuMeta = struct('rampTime', simu.rampTime, ...
                  'endTime',  simu.endTime,  ...
                  'dt',       simu.dt);
save(saveFile, 'simuMeta', '-append');
fprintf('\n已保存到 %s\n', saveFile);