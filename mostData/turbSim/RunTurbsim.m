...
clear
%% SETTINGS
WINDvector=8;                          % how many velocities of the wind to put 平均风速矢量（每个值都将创建一个风场文件）
Xgrid=-30:10:60;                       % m 风场域x方向拓展
time_breakpoints=[0 1000];             % Time breakpoints for with wind characteristics are defined 具有风向特征的时间断点已被确定
elevations=[0 0]*pi/180;               % for WINDvector with more than 1 element: concatenate on first dimension 不同时刻风速仰角
azimuths=[0 0]*pi/180;                 % for WINDvector with more than 1 element: concatenate on first dimension 不同时刻风速方位角
